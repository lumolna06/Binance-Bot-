...
REFRESCO_MINUTOS = 30    # Recalcular SAR cada 30 min de reloj
...
# Variables globales para control de actualización
ultimo_calculo_sar = 0
cached_closes = None
cached_sar = None


def obtener_sar_completo():
    """Obtiene velas y calcula SAR completo alineado a bloques de 30 min del reloj."""
    global ultimo_calculo_sar, cached_closes, cached_sar

    ahora = datetime.utcnow()
    # Redondear al inicio del bloque de 30 minutos
    minuto_ajustado = (ahora.minute // REFRESCO_MINUTOS) * REFRESCO_MINUTOS
    inicio_bloque = ahora.replace(minute=minuto_ajustado, second=0, microsecond=0)
    timestamp_bloque = int(inicio_bloque.timestamp())

    # Solo recalcular si cambió el bloque
    if cached_sar is None or timestamp_bloque != ultimo_calculo_sar:
        ...
        cached_closes = closes
        cached_sar = sar
        ultimo_calculo_sar = timestamp_bloque

        logging.info(f"🔄 SAR recalculado. Bloque inicio {inicio_bloque} UTC")
    else:
        logging.info("⏸ Usando SAR en caché (mismo bloque de 30 min)")

    return cached_closes, cached_sar
