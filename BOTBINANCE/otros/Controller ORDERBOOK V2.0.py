import pandas as pd
import numpy as np
import time
from datetime import datetime, timezone
from binance.client import Client
import subprocess

from EMA import EMARealtime  # Clase EMARealtime separada

# ===== CONFIGURACIÓN =====
API_KEY = "KNOHovcOWEDLeQkXh6xT27ZEwGnNN9r1kkBpefE36tgGpG7MyN7XjYv99byJG1xp"
API_SECRET = "81Q1smIhahcj7XjgJDfJwcp7v3mbb4MvQaLg42mF3R0TJO25GZRO84j6g6MDOMZ"

SYMBOL = "ETHUSDC"
INTERVAL = "30m"
EMA_PERIOD = 200
HISTORICAL_MULTIPLIER = 5
SAR_STEP = 0.02
SAR_MAX = 0.2
CONSISTENT_SIGNALS = 15

# ===== VARIABLES =====
pos_actual = None
ultima_senal = None
contador_senal = 0

client = Client(API_KEY, API_SECRET)


# ===== FUNCIONES =====
def get_historical_klines(symbol, interval, limit):
    """Descarga velas cerradas desde Binance"""
    try:
        klines = client.get_klines(symbol=symbol, interval=interval, limit=limit)
        df = pd.DataFrame(klines, columns=[
            'timestamp', 'open', 'high', 'low', 'close', 'volume', 'close_time',
            'quote_asset_volume', 'number_of_trades', 'taker_buy_base', 'taker_buy_quote', 'ignore'])
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms', utc=True)
        df[['open', 'high', 'low', 'close', 'volume']] = df[['open','high','low','close','volume']].astype(float)
        print(f"[DEBUG] {len(df)} velas cerradas descargadas correctamente.")
        return df[['timestamp','open','high','low','close','volume']]
    except Exception as e:
        print("❌ Error descargando velas:", e)
        return pd.DataFrame()


def detect_structure(df):
    """Detecta estructura de mercado"""
    df['HH'] = df['HL'] = df['LH'] = df['LL'] = False
    df['trend'] = pd.Series([None] * len(df), dtype=object)

    for i in range(2, len(df)-1):
        prev1, prev2 = df['high'].iloc[i-1], df['high'].iloc[i-2]
        high, low = df['high'].iloc[i], df['low'].iloc[i]
        if high > prev1 and prev1 > prev2: df.at[i,'HH'] = True
        elif high < prev1 and prev1 < prev2: df.at[i,'LH'] = True
        if low > prev1 and prev1 > prev2: df.at[i,'HL'] = True
        elif low < prev1 and prev1 < prev2: df.at[i,'LL'] = True
        if df.at[i,'HH'] or df.at[i,'HL']: df.at[i,'trend']='up'
        elif df.at[i,'LH'] or df.at[i,'LL']: df.at[i,'trend']='down'

    print("[DEBUG] Estructura de mercado detectada.")
    return df


def calculate_sar(df, step=SAR_STEP, max_af=SAR_MAX):
    """Calcula el Parabolic SAR"""
    sar = [df['low'][0]]
    trend = 1 if df['close'][1] > df['close'][0] else -1
    ep = df['high'][0] if trend==1 else df['low'][0]
    af = step
    for i in range(1, len(df)):
        prev_sar = sar[-1]
        if trend == 1:
            curr_sar = prev_sar + af * (ep - prev_sar)
            curr_sar = min(curr_sar, df['low'].iloc[i-1], df['low'].iloc[i-2] if i>1 else df['low'].iloc[i-1])
            if df['low'].iloc[i] < curr_sar:
                trend = -1
                curr_sar = ep
                ep = df['low'].iloc[i]
                af = step
            else:
                if df['high'].iloc[i] > ep:
                    ep = df['high'].iloc[i]
                    af = min(af + step, max_af)
        else:
            curr_sar = prev_sar + af * (ep - prev_sar)
            curr_sar = max(curr_sar, df['high'].iloc[i-1], df['high'].iloc[i-2] if i>1 else df['high'].iloc[i-1])
            if df['high'].iloc[i] > curr_sar:
                trend = 1
                curr_sar = ep
                ep = df['high'].iloc[i]
                af = step
            else:
                if df['low'].iloc[i] < ep:
                    ep = df['low'].iloc[i]
                    af = min(af + step, max_af)
        sar.append(curr_sar)

    df['SAR'] = sar
    df['SAR_trend'] = np.where(df['close'] > df['SAR'], 'up', 'down')
    print("[DEBUG] SAR calculado.")
    return df


def detect_patterns(df):
    """Detecta patrones de velas japonesas"""
    try:
        df['bullish_engulfing'] = (df['close']>df['open']) & (df['open'].shift(1)>df['close'].shift(1)) & (df['close']>df['open'].shift(1))
        df['bearish_engulfing'] = (df['close']<df['open']) & (df['open'].shift(1)<df['close'].shift(1)) & (df['close']<df['open'].shift(1))
        df['hammer'] = ((df['close']-df['low'])>2*(df['open']-df['close'])) & ((df['close']-df['open'])>0)
        df['shooting_star'] = ((df['high']-df['close'])>2*(df['close']-df['open'])) & ((df['close']-df['open'])>0)
        df['morning_star'] = ((df['close'].shift(2)<df['open'].shift(2)) & (df['close'].shift(1)<df['open'].shift(1)) & (df['close']>df['open'].shift(1)) & (df['close']>df['close'].shift(2)))
        df['evening_star'] = ((df['close'].shift(2)>df['open'].shift(2)) & (df['close'].shift(1)>df['open'].shift(1)) & (df['close']<df['open'].shift(1)) & (df['close']<df['close'].shift(2)))
        df['piercing_line'] = ((df['close'].shift(1)<df['open'].shift(1)) & (df['close']>df['open'].shift(1)) & (df['close']<df['open'].shift(1)+(df['open'].shift(1)-df['close'].shift(1))/2))
        df['dark_cloud_cover'] = ((df['close'].shift(1)>df['open'].shift(1)) & (df['close']<df['open'].shift(1)) & (df['close']>df['open'].shift(1)-(df['close'].shift(1)-df['open'].shift(1))/2))
        df['tweezer_top'] = (df['high'].shift(1).round(2)==df['high'].round(2)) & (df['close'].shift(1)<df['open'].shift(1))
        df['tweezer_bottom'] = (df['low'].shift(1).round(2)==df['low'].round(2)) & (df['close'].shift(1)>df['open'].shift(1))
        print("[DEBUG] Patrones de velas detectados.")
    except Exception as e:
        print("❌ Error detectando patrones:", e)
    return df


def generate_signals(df):
    """Genera señales y ejecuta el archivo de órdenes si hay confirmación"""
    global pos_actual, ultima_senal, contador_senal

    for i in range(1, len(df)):
        try:
            price = df['close'].iloc[i]
            ema = df['EMA200'].iloc[i]
            trend = df['trend'].iloc[i]
            sar_trend = df['SAR_trend'].iloc[i]
            timestamp = df['timestamp'].iloc[i]

            bullish_patterns = ['bullish_engulfing','hammer','morning_star','piercing_line','tweezer_bottom']
            bearish_patterns = ['bearish_engulfing','shooting_star','evening_star','dark_cloud_cover','tweezer_top']

            bullish = any(df[p].iloc[i] for p in bullish_patterns)
            bearish = any(df[p].iloc[i] for p in bearish_patterns)

            direction = None
            if price > ema and sar_trend == 'up' and bullish and trend == 'up':
                direction = 'long'
            elif price < ema and sar_trend == 'down' and bearish and trend == 'down':
                direction = 'short'

            # Solo debug al final del procesamiento de vela
            print(f"[DEBUG] Procesando vela: {timestamp} - Precio: {price:.2f}, EMA200: {ema:.2f}, SAR_trend: {sar_trend}, Trend: {trend}, Señal: {direction or 'NINGUNA'}")

            if direction:
                if ultima_senal == direction:
                    contador_senal += 1
                else:
                    contador_senal = 1
                    ultima_senal = direction

                if contador_senal >= CONSISTENT_SIGNALS and pos_actual is None:
                    pos_actual = direction
                    print(f"[SEÑAL {direction.upper()}] {timestamp} - Precio: {price:.2f}, EMA: {ema:.2f}")
                    contador_senal = 0

                    try:
                        subprocess.run(["python", "Binance ORDERBOOK V.2.0.py"], check=True)
                        print("[INFO] Script Binance ORDERBOOK V.2.0 ejecutado correctamente.")
                    except Exception as e:
                        print("❌ Error ejecutando Binance ORDERBOOK V.2.0:", e)

        except Exception as e:
            print("❌ Error procesando vela:", e)


def es_nueva_vela(interval=30):
    """Verifica si hay una nueva vela cerrada"""
    ahora = datetime.now(timezone.utc)
    return (ahora.minute % interval == 0) and (ahora.second < 10)


# ===== LOOP PRINCIPAL =====
if __name__ == "__main__":
    INITIAL_LIMIT = EMA_PERIOD * HISTORICAL_MULTIPLIER

    try:
        ema_handler = EMARealtime(client, SYMBOL, interval=INTERVAL, length=EMA_PERIOD, data_limit=1500)
        ema_handler.inicializar_ema()
        print("[DEBUG] EMA inicializada correctamente.")
    except Exception as e:
        print("❌ Error inicializando EMA:", e)

    last_timestamp = None

    while True:
        ahora = datetime.now(timezone.utc)
        if es_nueva_vela():
            try:
                print(f"[DEBUG] Nueva vela detectada: {ahora}. Descargando historial...")
                df = get_historical_klines(SYMBOL, INTERVAL, INITIAL_LIMIT)
                if df.empty:
                    time.sleep(10)
                    continue

                # EMA200 calculada solo por la clase EMARealtime
                df['EMA200'] = [float(ema_handler.actualizar_por_trade(p)) for p in df['close']]

                df = detect_structure(df)
                df = detect_patterns(df)
                df = calculate_sar(df)

                if last_timestamp is None:
                    last_timestamp = df['timestamp'].iloc[-1]
                    print(f"[DEBUG] Inicializando last_timestamp: {last_timestamp}")
                    time.sleep(10)
                    continue

                new_df = df[df['timestamp'] > last_timestamp]
                if not new_df.empty:
                    print(f"[DEBUG] Procesando {len(new_df)} velas nuevas cerradas...")
                    generate_signals(new_df)
                    last_timestamp = new_df['timestamp'].iloc[-1]
                    print(f"[DEBUG] Última vela procesada actualizada: {last_timestamp}")

            except Exception as e:
                print("❌ Error en el procesamiento de la vela:", e)

        else:
            print(f"[DEBUG] Esperando cierre de vela... Hora actual: {ahora.strftime('%H:%M:%S')}")

        time.sleep(10)
